...
def generate_launch_description():
    ...
    yaren_description_share = os.path.join(get_package_prefix("yaren_description"), "share")
    gazebo_ros_dir = get_package_share_directory("gazebo_ros")

    # Argumento de lanzamiento para especificar la ubicación del archivo URDF del robot
    model_arg = DeclareLaunchArgument(
        name="model",
        default_value=os.path.join(yaren_description_dir, "urdf", "yaren_description.urdf.xacro"),
        description="Absolute path to robot urdf file"
    )

    # Declaración del archivo del mundo de Gazebo
    world_file_name = 'mundo.world'
    world_path = os.path.join(yaren_description_dir, 'worlds', world_file_name)
    ...
    env_var = SetEnvironmentVariable("GAZEBO_MODEL_PATH", yaren_description_share)
